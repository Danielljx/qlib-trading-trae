...
def generate_report(metrics_df, pm_df, trade_df, config=None):
    ...
    lines.append("  Name:       BTC Short-Term v1")
    lines.append("  Instrument: BTCUSDT")
    lines.append("  Frequency:  1 Hour (60min)")
    lines.append("  Model:      LightGBM (Huber Loss, alpha=0.9)")
    lines.append("  Features:   35")
    lines.append("  Label:      Future 1H Return (Ref($close,-2)/Ref($close,-1)-1)")
    lines.append("  Strategy:   ThresholdSignalStrategy (long=0.1%, short=-0.1%)")
    lines.append("")
